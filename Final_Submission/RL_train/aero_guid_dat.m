...
% =================================================================
% RL Target Evasion Settings (추가 파라미터)
% =================================================================
target_dt = 0.01;            % RL 및 Target 제어 주기 [s]

R_RL_enter = 3500;           % RL 회피 모드 진입 거리 [m]
R_RL_exit  = 4200;           % 회피 성공 판정 거리 [m]
R_capture  = 50;            % 미사일 포착/명중(실패) 거리 [m]

Goal_pos = [4800; -3200];

R_scale = 5000;              % Observation 정규화용 거리 Scale
V_scale = 1000;              % Observation 정규화용 속도 Scale

target_max_turn_rate = 120 * (pi/180); % Target 최대 선회율 [rad/s] (60 deg/s)

max_episode_time = 25.0;   % Maximum episode time [s]

% 초기화 파일 재실행
disp('RL parameters loaded successfully.');
